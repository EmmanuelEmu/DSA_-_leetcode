...
bool solve(int ind1, int ind2, string &s, string &p, vector<vector<int>> &dp)
{
    ...
    bool match = (ind1 < s.size() && (s[ind1] == p[ind2] || p[ind2] == '.'));

    // Handle the '*' character in the pattern
    if (ind2 + 1 < p.size() && p[ind2 + 1] == '*')
    {
        // Case 1: '*' matches zero occurrences of the preceding element
        bool skipStar = solve(ind1, ind2 + 2, s, p, dp); // Skip the preceding element and '*'
        // Case 2: '*' matches one or more occurrences of the preceding element
        bool matchStar = match && solve(ind1 + 1, ind2, s, p, dp); // Move string pointer, keep pattern
...
{
    string s1, s2;
    cin >> s1 >> s2;                                      // Input string and pattern
    cout << (isMatch(s1, s2) ? "true" : "false") << endl; // Output result
    return 0;
}
