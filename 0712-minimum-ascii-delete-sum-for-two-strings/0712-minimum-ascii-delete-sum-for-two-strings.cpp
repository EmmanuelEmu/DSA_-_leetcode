...
public:
    ...
    string findMaxAsciiLCS(string& text1, string& text2) {
        ...
        // DP table to store the maximum ASCII sum of LCS for substrings
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= m; j++) {
                if (text1[i - 1] == text2[j - 1]) {
                    ...
                    // Otherwise, take the maximum ASCII value sum from the
                    // previous states
        ...
        while (i > 0 && j > 0) {
            if (text1[i - 1] == text2[j - 1]) {
                maxAsciiLCS +=
                    text1[i - 1]; // If characters match, add it to the result
                i--;
                j--;
        ...
        reverse(maxAsciiLCS.begin(),
                maxAsciiLCS.end()); // LCS was built backwards, reverse it
        return maxAsciiLCS;
    }
